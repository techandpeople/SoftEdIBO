...
class WifiOTAUpdater:
    ...
    def _stage_failure_message(self) -> str:
        ...
        return "Staging failed: " + _STAGE_ERRORS.get(reason, reason)
    ...
    def _stage(self, image: bytes, md5: str) -> str | None:
        ...
        for off in range(0, total, STORE_CHUNK):
            if self._cancelled or self._error:
                return None
            chunk = base64.b64encode(image[off:off + STORE_CHUNK]).decode("ascii")
            if not self._gateway.send_gateway("ota_store_data", data=chunk):
                self._error = "usb_write_failed"
                return None
            pct = min(99, int((off + STORE_CHUNK) * 100 / total))
    ...
    def _handle(self, data: dict[str, Any]) -> None:
        ...
        elif source == self._mac and mtype == "ota_wifi_start":
            self._started = True
        elif source == self._mac and mtype == "ota_done":
            self._done = True
        elif source == self._mac and mtype == "ota_error":
