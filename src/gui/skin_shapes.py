...
``thymio`` (a 'D' rotated -90 deg: flat top, semicircular bottom).
"""
...
def shape_path(shape: str, rect: QRectF) -> QPainterPath | None:
    ...
    if shape == "triangle":
        ...
        return path
    if shape == "thymio":
        # 'D' rotated -90 deg: semicircular bulge on TOP, flat bottom edge.
        path = QPainterPath()
...
def aspect_box(size_mm: tuple[float, float] | None,
    ...
    """Largest box of the skin's physical aspect ratio that fits ``widthxheight``,
