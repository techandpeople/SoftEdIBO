...
| File | Role |
|------|------|
| `src/hardware/quadrant_detector.py` | Signal processing — absolute µT thresholds + Schmitt-trigger hysteresis per sensor, position estimation from active sensors |
...
```
node_magnet_sensor (shared firmware/common/se_magnet.h)
  → ESP-NOW → Gateway
    → ESP32Controller._handle_message()
      → _dispatch_magnet(data)
        ├── SkinGridView._on_magnet_msg()   ← GUI: flash yellow (raw stream)
        └── skin.touch_source               ← CompensatedMagnetSource when
            │                                 compensation is on, else raw
            ├── Skin._on_magnet_touch_data()      ← feed QuadrantDetector
            │     → TouchPositionTracker.update()
            │       → skin.get_touch_position()   ← polled by activities
            ├── TouchEventRouter.handle_magnet()  ← press/release per chamber
            │     → skin.on_touch_event(cb)
            └── subscribe_skin_magnet(skin, cb)   ← activities & gesture ML
```
...
| `act` | Indices of sensors whose `mag` is at/above the firmware threshold (`act_threshold_ut`, default 300 µT) |
| `vec` | Optional per-sensor 3-axis deltas `[[dx,dy,dz], …]` (µT) when vector streaming is on (`{"cmd":"configure","stream_vec":true}` or the `MAG_VECTOR` build) — feeds vector compensation and the gesture ML's direction features |
...
touch:
  ...
  # Detection tuning (all optional; raw µT units)
  quadrant_thresholds: [100, 100, 100, 100]  # per-sensor activation, µT
  hysteresis: 20                    # µT below threshold before deactivation
...
| `quadrant_thresholds` | `[100, …]` µT | One threshold per sensor; lower = more sensitive.  Tune live in the Touch tuning panel |
| `hysteresis` | `20` µT | Band below threshold before deactivation |
...
/ `set_touch_hysteresis()` adjust the detector at runtime (µT);
...
|---------|--------------|-----|
| No touch detected | Baseline still settling, or threshold too high | Wait for the boot auto-zero (~70 reads), press **Re-zero sensors**, lower `quadrant_thresholds` (µT) |
...
| Flickering on/off | Sensor noise at edge of threshold | Increase `hysteresis` (try 30–50 µT) |
