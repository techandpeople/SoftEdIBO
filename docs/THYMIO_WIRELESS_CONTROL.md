...
| prox.ground.delta | 0x54 (84) | 2 | | temperature (x10  degC) | 0x76 (118) | 1 |
...
- **Sound - DONE (2026-07-03).** `sound.system`/`sound.freq`/`sound.play` are *native
  ...
  it: **SET_BYTECODE (`0xA001`) then RUN (`0xA003`)**. (! NOT `0xA000`/`0xA002` - those are
...
|------|:----:|:---:|:--------:|-------|
| ESP32-S3 | yes | yes | **no** | `SOC_IEEE802154_SUPPORTED` absent - radio not in silicon |
| ESP32-C6 | yes | yes | **yes** | `SOC_IEEE802154_SUPPORTED=1` (Wi-Fi 6 + BLE 5 + 802.15.4) |
| ESP32-H2 | no | yes | yes | 802.15.4 + BLE, **no Wi-Fi** |
| ESP32-C5 | yes | yes | yes | dual-band Wi-Fi 6 + BLE + 802.15.4, supports PSRAM |
...
- **The Thymio's main MCU does not do the 802.15.4 framing itself.** It drives an
  **external RF module over I^2C** (addr `0x42`): registers for PAN/network id
  ...
  module's own firmware. **So the Thymio is already a host+RCP split (MCU <-> I^2C <->
...
- **Logical channels 0/1/2 (default 1)** are the RF module's encoding, *not* raw
  ...
  firmware. Reference: Retornaz et al., *Seamless Multi-Robot Programming... Wireless
  Thymio II*, 2013.
