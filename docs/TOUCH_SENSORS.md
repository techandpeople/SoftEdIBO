...
|-------|------------------|
| `TouchEventRouter` (press/release per chamber from the `act` index set) | yes generic |
| `on_magnet` / `subscribe_skin_magnet` fan-out, `SimulatedMagnetSensor` | yes generic dict |
| `on_touch_event(chamber, action)` consumed by activities + GUI | yes generic |
| Skin <-> touch-node config (`touch.node_mac`, `sensor_count`, `sensor_grid`) | yes generic |
| Wire strings (`type:"magnet"`, `node_magnet_sensor_ready`), signal units | no per-sensor |
| Spatial detector (`QuadrantDetector`, uT thresholds, 4-sensor geometry) | no per-sensor |
| Pressure->touch compensation (magnet shifts in silicone) | no per-sensor |
...
- **`MagnetSensorProfile`** - the only technology shipped; wraps the uT
