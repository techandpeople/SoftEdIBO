...
The grid's shape/size comes from the skin geometry (2×2 'D' for the Thymio, a
plain 2×2 fallback, or an explicit bigger grid for future capacitive touch), and
...
def test_thymio_geometry_yields_2x2_d_grid(qtbot):
    # Four sensors in a 2×2: top pair in the bulge, bottom pair on the flat edge.
...
def test_active_sensors_track_the_threshold(qtbot):
    ...
    assert grid.active_sensors() == {1, 3}         # only ≥100 µT
