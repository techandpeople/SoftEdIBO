...
| `hold_duty` | `chamber` (-1 = all), `duty` (180-255), `kpa`?, `dir`?, `timed`?, `off`? | Leak-compensating regulated hold on the chamber's own gauge. `dir` 0 (default) = pressure hold: once the reading drifts 0.25 kPa below `kpa` the INFLATE valve opens and the pressure pump tops the chamber up in a short pulse; `dir` 1 = vacuum hold: the mirror with the DEFLATE valve and the vacuum pump. Pulses are kept soft: the valve closes when the reading extrapolated 80 ms ahead reaches the target (2-tick debounce, 60 ms minimum pulse), the pump duty ramps up (2 PWM/ms) instead of stepping, and the shared duty is `base + 25/kPa * deficit` where `base` decays one step toward the 180 PWM floor (`pump_duty::MIN`) on every completed pulse and climbs only while a pulse starves (> 1.5 s open). Chambers pulse one target level at a time: only targets within 0.5 kPa of the level on the line open together, the rest wait (a starving line admits everyone). A vacuum target below the gauge floor (blind 0..100 sensor) is held by timed re-pulls instead: 120 ms at `duty` every 2.5 s, plus an immediate pull whenever the chamber leaks up into the visible band. `duty` seeds the servo (or is the re-pull duty). `timed:1` (or no `kpa`) keeps the valve open at `duty` (sensorless boards). Pumps off whenever no held valve of their side is open. PC must re-send ~2 s as keepalive (6 s dead-man drops the hold). `off:1` drops it. Any inflate/deflate/set_pressure/hold on the chamber supersedes it; fill engines/manual/bench runs suspend it and it resumes after |
